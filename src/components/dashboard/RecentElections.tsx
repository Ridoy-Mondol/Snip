...
export default function RecentElections({ election }: ElectionsProps) {
      ...
      <CardHeader
        title={
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <MdOutlineHowToVote size={24} color={theme.palette.primary.main} />
      ...
      <CardContent sx={{ p: 0 }}>
        {election.length === 0 ? (
          ...
          <Grid container direction="column">
            {election.slice(-2).map((election: any, index: number) => (
              <Grid item key={ index } sx={{ width: '100%' }}>
                <NextLink href={`/dao/elections/${election.electionName}`} passHref style={{ textDecoration: 'none' }}>
                  <Box
                    sx={{
                      display: "flex",
                ...
                {index < election.length - 1 && <Divider component="li" sx={{ ml: 2, mr: 2 }} />}
              </Grid>
            ))}
          </Grid>
        )}
